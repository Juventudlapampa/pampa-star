...
## ✅ ADDENDUM V6: OPENING + MÚSICA (misma noche) — COMPLETO
`docs/ADDENDUM_V6_OPENING_Y_MUSICA.md`. **B (reemplaza §6, `3fb413a`)**: música con DIRECCIÓN — EL MOTIVO 1-5-6-8 en todos los temas (insinuado/completo/invertido/principal), tabla de temas por tonalidad-tempo-carácter, progresión i–VI–III–VII, 3 capas + viento pampeano, stingers sobre el motivo, todo en `balance.musica`. **A (`65a4bc5`)**: `scenes/intro.js` — el opening de 8 planos (~20s, cortes secos, salteable, 1×sesión, "▶ VER INTRO" en el editor, `balance.intro`). Primera vista de la sesión puede ser muda (autoplay del navegador).

## ✅ MAESTRO V6 "EL ANIME JUGANDO" (16-17/jul/2026) — EL ORDEN DEL §9 COMPLETO (11/11)
`docs/DISENO_MAESTRO_V6.md` (consolida y reemplaza v2/v3/v4) + `docs/PAMPA_STAR_GDD.md` como documento raíz + **las 7 poses ilustradas de Rodri** (`assets/poses/`, chroma→alpha, manifest tolerante). Un commit por bloque `ce9d4e5`→`3bbdbef`, revisión inline, suite **2.322**, soak final 94' con todo integrado y 0 errores: **1** fixes F1-F6 (radar de VUELTA, arquero siempre vuela, corredor de intercepción, cambio excluye al que perdió, marcas, GUTS→AGUANTE hasta en el clásico) · **2** EL CAMBIO DE RAÍZ: modelo de saltos (nadie persigue: test de corrección del §0 en la suite), separación post-duelo, reloj POR MOMENTOS con presets pre-partido (RELÁMPAGO/INTERMEDIO/LARGO + velocidad + SKIP) y **LA CEGUERA** (los rivales no se dibujan: solo radar, cruce y escenas; `radio_revelacion` de rescate) · **3** **LA DEFINICIÓN v2** ofensiva Y defensiva (4 fases: posición corriendo, 6 zonas con etiqueta + aguja, vuelo con poses, freeze+silencio+revelación; la atajada se grita como gol) · **4** animación limitada en el gestor (pose quieta + todo lo demás en movimiento, corte seco) · **5** aguante del original (Caldén 450 = dos y chau; CPU con límite invisible) · **6** chiptune con percusión + la crecida · **7** medidor de **ENVIÓN** (potenciar / súper defensa) · **8** megacosas al gestor + hinchada saltando · **9** relator (ya cumplido) · **10** **SECUENCIAS** (megacorrida encadenada + jugada combinada donde el compañero define) · **11** modo **MASTER** (5 divisiones fijas + perfiles de IA por club). **👉 LEER `HANDOFF_V6.md`**: checklist para el celu + "LO QUE FALTA PARA MAÑANA" (poses nuevas, playtest de ceguera/tempo, carrera al Phaser).
